...
impl Fen {
    ...
    pub fn compute_fen(state: &BoardState) -> String {
...
mod tests {
    ...
    fn from_fen_developed() {
        let bitboard = Bitboard::new();
        let state = Fen::from_fen_default("r2q1rk1/pbp2ppp/1pnp1n2/8/2PPp3/2P1P3/P1N2PPP/R1BQKB1R w kq - 0 10");
        assert_eq!(state.to_string(), "+---+---+---+---+---+---+---+---+\n\
                                       | r |   |   | q |   | r | k |   |\n\
                                       +---+---+---+---+---+---+---+---+\n\
                                       | p | b | p |   |   | p | p | p |\n\
    ...
    fn to_fen_no_castling() {
        ...
        let state = Fen::from_fen_default(fen_without_castling);
        let fen = Fen::compute_fen(&state);
    ...
    fn to_fen_with_en_passant() {
        ...
        let state = Fen::from_fen_default(fen_with_en_passant);
        let fen = Fen::compute_fen(&state);
        println!("{}", fen);
